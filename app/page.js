    ...
    <div style={{ margin: 0, background: C.bg, color: '#fff', fontFamily: "'Inter',-apple-system,sans-serif", minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px 20px' }}>
      <div style={{ background: C.surf, border: `1px solid ${C.bord}`, borderRadius: 24, padding: '32px 20px', width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginBottom: 24 }}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{ width: 16, height: 16, borderRadius: '50%', background: i < pin.length ? C.gold : '#2a2a35', border: `1px solid ${i < pin.length ? C.gold : '#3a3a48'}`, transition: 'all .15s' }} />
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '14px 14px 90px' }}>
          ...
          <div style={card}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <input
                style={{ flex: 1, minWidth: 0, background: C.surf2, border: `1px solid ${C.bord}`, borderRadius: 10, color: '#fff', fontSize: 18, padding: '12px 14px', outline: 'none', fontFamily: 'inherit' }}
            ...
            {!dash?.cashOutEntries?.length
              ...
              : <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, minWidth: 320 }}>
                    <thead>
                      <tr>
                        {['Time','Who','Amount','Reason', user.role==='admin' ? '' : null].filter(h => h !== null).map((h,i) => (
                          <th key={i} style={{ background: C.surf2, color: C.muted, fontSize: 11, letterSpacing: 1, padding: '10px 12px', textAlign: 'left', textTransform: 'uppercase', borderBottom: `1px solid ${C.bord}`, whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {dash.cashOutEntries.map((e, i) => (
                        <tr key={i}>
                          ...
                          {user.role === 'admin' && (
                            <td style={{ padding: '11px 12px', borderBottom: `1px solid rgba(255,255,255,.05)` }}>
                              <button onClick={() => delEntry(e.rowIndex)} style={{ background: 'none', border: `1px solid ${C.bord}`, borderRadius: 6, color: C.muted, cursor: 'pointer', fontSize: 12, padding: '4px 8px' }}>✕</button>
                            </td>
                          )}
                        </tr>
                      ))}
          ...
          <div style={card}>
            ...
            <div style={fld}>
              ...
              <select style={inp} value={outReason} onChange={e => { setOutReason(e.target.value); if (e.target.value !== 'Employee Pay') setOutHours(''); }}>
                ...
                <option>Store Expense</option>
                <option>Employee Pay</option>
            ...
            {outReason === 'Employee Pay' && (
              <div style={fld}>
                <label style={flbl}>Hours Worked</label>
                <input style={inp} type="number" placeholder="e.g. 8" inputMode="decimal" value={outHours} onChange={e => setOutHours(e.target.value)} />
              </div>
            )}
            <div style={fld}>
              <label style={flbl}>Note (optional)</label>
              <input style={inp} type="text" placeholder="Add details..." value={outNote} onChange={e => setOutNote(e.target.value)} />
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '14px 14px 90px' }}>
          ...
          {rptLoading && (
            <div style={{ textAlign: 'center', padding: 40, color: C.muted, fontSize: 14 }}>
              <div style={{ width: 24, height: 24, border: `2px solid ${C.bord}`, borderTopColor: C.gold, borderRadius: '50%', animation: 'spin .8s linear infinite', margin: '0 auto 12px' }} />
              Loading...
            </div>
          )}
              ...
              {rptData.days && rptData.days.length > 0 && (
                <div style={{ background: C.surf, border: `1px solid ${C.bord}`, borderRadius: 14, overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, minWidth: 340 }}>
                    <thead>
                      <tr>
                        {['Date','Cash In','Txn','Out','Diff'].map(h => (
